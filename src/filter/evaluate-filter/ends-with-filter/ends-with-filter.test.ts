...
describe('EndsWithFilter', () => {
  it('string', () => {
    ...
    expect(
      filter.findMany({ where: { name: { endsWith: 'ice' } } }).length
    ).toBe(2)
    expect(
      filter.findMany({ where: { name: { endsWith: 'Cerati' } } }).length
    ).toBe(1)
    expect(
      filter.findMany({ where: { name: { endsWith: 'David' } } }).length
    ).toBe(1)
    expect(
      filter.findMany({ where: { name: { endsWith: 'smi' } } }).length
    ).toBe(0)
    expect(filter.findMany({ where: { name: { endsWith: ' ' } } }).length).toBe(
      0
    )
