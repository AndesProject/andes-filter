...
describe('NotContainsFilter', () => {
  it('string', () => {
    ...
    expect(
      filter.findMany({ where: { name: { notContains: 'Bob' } } }).length
    ).toBe(11)
    expect(
      filter.findMany({ where: { name: { notContains: 'Mariana' } } }).length
    ).toBe(12)
    expect(
      filter.findMany({ where: { name: { notContains: 'Alice' } } }).length
    ).toBe(10)
    expect(
      filter.findMany({ where: { name: { notContains: 'eva' } } }).length
    ).toBe(12)
    expect(
      filter.findMany({ where: { name: { notContains: 'ank' } } }).length
    ).toBe(11)
    expect(
      filter.findMany({ where: { name: { notContains: ' ' } } }).length
    ).toBe(11)
    expect(
      filter.findMany({ where: { name: { notContains: '' } } }).length
    ).toBe(0)
