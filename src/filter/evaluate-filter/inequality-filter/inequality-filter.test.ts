...
describe('InequalityFilter', () => {
  ...
  it('number', () => {
    ...
    expect(filter.findMany({ where: { size: { not: undefined } } }).length).toBe(5)

    expect(filter.findUnique({ where: { size: { not: undefined } } })?.size).toBe(1)
  ...
  it('boolean', () => {
    ...
    expect(filter.findMany({ where: { isValid: { not: true } } }).length).toBe(1)
    expect(filter.findMany({ where: { isValid: { not: false } } }).length).toBe(2)
    expect(filter.findMany({ where: { isValid: { not: null } } }).length).toBe(3)
    expect(filter.findMany({ where: { isValid: { not: undefined } } }).length).toBe(3)

    expect(filter.findUnique({ where: { isValid: { not: undefined } } })?.isValid).toBe(true)
    expect(filter.findUnique({ where: { isValid: { not: true } } })?.isValid).toBe(false)
    expect(filter.findUnique({ where: { isValid: { not: false } } })?.isValid).toBe(true)
  })
})
