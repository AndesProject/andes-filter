...
describe('InequalityFilter Unit', () => {
  ...
  it('debe manejar objetos con operadores de some', () => {
    ...
    // not: { some: { equals: 2 } } ≡ none: { equals: 2 }
    expect(filter.evaluate([1, 2, 3])).toBe(false) // tiene un 2
    expect(filter.evaluate([1, 3, 4])).toBe(true) // ninguno es 2
  })

  it('debe manejar objetos con operadores de none', () => {
    const filter = new InequalityFilter<any>({ none: { equals: 2 } })
    // not: { none: { equals: 2 } } ≡ some: { equals: 2 }
    expect(filter.evaluate([1, 3, 4])).toBe(false) // ninguno es 2
    expect(filter.evaluate([1, 2, 3])).toBe(true) // tiene un 2
  })

  it('debe manejar objetos con operadores de every', () => {
    const filter = new InequalityFilter<any>({ every: { equals: 2 } })
    // not: { every: { equals: 2 } } ≡ some: { not: { equals: 2 } }
    expect(filter.evaluate([2, 2, 2])).toBe(false) // todos son 2
    expect(filter.evaluate([1, 2, 3])).toBe(true) // algunos no son 2
    expect(filter.evaluate([1, 3, 4])).toBe(true) // ninguno es 2
  })
  ...
  it('debe manejar objetos anidados por referencia', () => {
    ...
    // Misma referencia - debe ser false (igual)
    expect(filter.evaluate(obj1)).toBe(false)
    // Diferente objeto aunque tenga misma referencia interna - debe ser true (diferente referencia del objeto contenedor)
    expect(filter.evaluate(obj2)).toBe(true)
    // Diferente objeto con mismo contenido - debe ser true (diferente referencia)
